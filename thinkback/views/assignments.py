...
from ..models import Assignment, UploadedFile, ProblemTestCases
...
# TODO: Delete this, working on database
# This is used for testing the various routes
assignment_list = []
assignment1 = Assignment(
	"Basics in python", "People search for this Lorem ipsum dummy copy text using all kinds of names, such as Lorem ipsum, lorem ipsum dolor sit amet, Lorem, dummy text, loren ipsum (yes, spelled wrong)", True)
...
assignment2 = Assignment("Advance mathematics in python",
						 "I’ve seen several versions of this Lorem ipsum text on the Internet, with various changes; however, this Lorem Ipsum dummy copy text doesn’t have any odd text sneaked in as I’ve seen in some of the others.", True)
...
assignment3 = Assignment(
	"Number Lists", "There is an unformatted plain text (.txt) version to either copy/paste or to save the .txt file to your computer. Note: it’s unformatted and will stretch across your screen. ", False)
...
def upload_file(link, problem_id):
	if request.method == 'POST':
		...
		file_uploaded = UploadedFile(file)
		if file_uploaded.is_empty():
			flash("Something went wrong")
			return redirect(request.url)

		if file_uploaded.file and file_uploaded.is_allowed():
			filename = secure_filename(file_uploaded.file.filename)
			path = file_uploaded.create_file_path(problem_id)

			if not os.path.exists(path):
				os.makedirs(path)

			file_uploaded.save_file_to_path(path, filename)
			module_path = '.uploads.{}'.format(problem_id)
			problem_module = file_uploaded.get_file_module(module_path, filename)
			function_name = get_problem_info(problem_id).function
			problem_function = getattr(problem_module, function_name)
			tmp = ProblemTestCases(correct_func)
			tmp.run_tests(problem_function)
			return redirect(request.url)
